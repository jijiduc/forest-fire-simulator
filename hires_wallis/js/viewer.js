...
class ForestFireViewer {
    ...
    setupEventListeners() {
        ...
        if (viewModeSelect) {
            viewModeSelect.addEventListener('change', (e) => {
                this.grid2D.setViewMode(e.target.value);
    ...
    updateLegend(viewMode) {
        // Hide all legends
        const legends = ['stateLegend', 'vegetationLegend', 'moistureLegend', 'temperatureLegend'];
        legends.forEach(id => {
        ...
        switch (viewMode) {
            case 'vegetation':
                const vegLegend = document.getElementById('vegetationLegend');
                if (vegLegend) vegLegend.style.display = 'block';
                break;
            case 'moisture':
                const moistLegend = document.getElementById('moistureLegend');
                if (moistLegend) moistLegend.style.display = 'block';
                break;
            case 'temperature':
                const tempLegend = document.getElementById('temperatureLegend');
                if (tempLegend) tempLegend.style.display = 'block';
                break;
            case 'state':
            default:
                const stateLegend = document.getElementById('stateLegend');
                if (stateLegend) stateLegend.style.display = 'block';
                break;
        }
    }
}
