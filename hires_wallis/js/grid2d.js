...
export class Grid2DRenderer {
    constructor(canvas) {
        ...
        // Store current cells data
        this.currentCells = [];
        
        // Options
        this.showGrid = true;
        this.showElevation = false;
        this.viewMode = 'state'; // 'state', 'vegetation', 'moisture', 'temperature'
        ...
        this.temperatureGradient = [
            ...
            '#64b5f6', '#90caf9', '#bbdefb', '#ffeb3b', '#ffc107',
            '#ff9800', '#ff5722', '#f44336', '#d32f2f', '#b71c1c'
        ];
        
        // Moisture gradient (brown to blue)
        this.moistureGradient = [
            '#3e2723', '#4e342e', '#5d4037', '#6d4c41', '#795548',
            '#8d6e63', '#a1887f', '#b39ddb', '#9fa8da', '#90caf9',
    ...
    render(cells = null) {
        // Use provided cells or stored cells
        if (cells !== null) {
    ...
    getCellColor(cell) {
        switch (this.viewMode) {
            case 'vegetation':
                return this.getVegetationColor(cell);
            
            case 'moisture':
                return this.getMoistureColor(cell);
            
            case 'temperature':
                return this.getTemperatureColor(cell);
            
            case 'state':
            default:
                return this.getStateColor(cell);
        }
    }
    ...
    getVegetationColor(cell) {
        if (cell.state === 'Burning') {
            // Show burning state even in vegetation mode
            const intensityIndex = Math.min(3, Math.floor(cell.fireIntensity * 4));
            return this.colors.burning[intensityIndex];
        }
        
        return this.vegetationColors[cell.vegetation] || this.colors.empty;
    }
    
    getMoistureColor(cell) {
        if (cell.state === 'Burning') {
            // Show burning state even in moisture mode
            const intensityIndex = Math.min(3, Math.floor(cell.fireIntensity * 4));
            return this.colors.burning[intensityIndex];
        }
        
        // Map moisture (0-1) to gradient index
        const index = Math.min(14, Math.floor(cell.moisture * 15));
        return this.moistureGradient[index];
    }
    
    getTemperatureColor(cell) {
        if (cell.state === 'Burning') {
            // Show burning state even in temperature mode
            const intensityIndex = Math.min(3, Math.floor(cell.fireIntensity * 4));
            return this.colors.burning[intensityIndex];
        }
        
        // Map temperature (assume 0-50°C range) to gradient index
        const normalizedTemp = Math.max(0, Math.min(1, cell.temperature / 50));
        const index = Math.min(14, Math.floor(normalizedTemp * 15));
        return this.temperatureGradient[index];
    }
